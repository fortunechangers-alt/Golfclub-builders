...
(function() {
    ...
    function initScrollAnimation() {
        console.log('Initializing scroll animation. Duration:', video.duration);
        
        function updateVideo() {
            // Calculate scroll progress based on page scroll
            // For 1-second video, complete animation over ~1.5 screen heights
            const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
            const animationDistance = window.innerHeight * 1.5; // Animation completes after 1.5 screen heights
            const scrollProgress = Math.max(0, Math.min(1, scrollTop / animationDistance));
            
            // Update video currentTime based on scroll
            const videoDuration = video.duration;
            const newTime = videoDuration * scrollProgress;
            
            if (Math.abs(video.currentTime - newTime) > 0.01) {
                try {
                    video.currentTime = newTime;
                    console.log('Scroll progress:', scrollProgress.toFixed(2), 'Video time:', newTime.toFixed(2), 'Seeking:', video.seeking, 'ReadyState:', video.readyState);
                } catch(e) {
                    console.error('Error setting video time:', e);
                }
            }
            
            // Play sound at 95% completion
            if (scrollProgress >= 0.95 && !soundPlayed && sound) {
            ...
            // Reset sound flag when scrolling back up
