...
(function() {
    ...
    function unlockAudio() {
        ...
        if (playPromise !== undefined) {
            playPromise.then(() => {
                ...
                console.log('Audio unlock failed (will retry):', e.message);
            });
        }
    }
    
    // Aggressive unlock: Try on scroll WITHOUT passive mode
    let scrollUnlockAttempted = false;
    window.addEventListener('scroll', function unlockOnScroll() {
        if (!scrollUnlockAttempted) {
            scrollUnlockAttempted = true;
            unlockAudio();
        }
    }, { once: true });
    
    // Backup: also try on click/touch
    ...
    function initScrollAnimation() {
        ...
        function updateAnimation() {
            ...
            // Unlock audio on first scroll (early, before we need it)
            ...
            // Play sound at 75% completion (audio should be unlocked by now)
            if (scrollProgress >= 0.75 && !soundPlayed && sound && audioUnlocked) {
