...
(function() {
    ...
    // Try to unlock audio IMMEDIATELY on first scroll (synchronously!)
    let unlockAttempted = false;
    
    function attemptAudioUnlock() {
        if (unlockAttempted || audioUnlocked || !sound) return;
        ...
        try {
            ...
            if (playPromise !== undefined) {
                playPromise.then(() => {
                    sound.pause();
                    sound.currentTime = 0;
                    sound.volume = 1.0; // Restore volume
                    audioUnlocked = true;
                    console.log('✅ Audio unlocked on scroll!');
                }).catch(() => {
                    sound.volume = 1.0; // Restore volume even if failed
                });
            }
        } catch(e) {
            sound.volume = 1.0;
    ...
    function initScrollAnimation() {
        ...
        function updateAnimation() {
            ...
            // Try to unlock audio on first animation update (happens during scroll)
            attemptAudioUnlock();
